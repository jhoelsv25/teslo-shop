    ...
    <aside className="">
      ...
      {isSideMenuOpen && (
        <>
          ...
          {/* BLUR */}
          <div onClick={closeSideMenu} className="fixed inset-0 z-10 backdrop-blur-xs backdrop-filter"></div>
        </>
      )}
      {/* Sidebar content */}
      <nav
        className={clsx(
          "fixed p-5 top-0 right-0 w-125 h-screen bg-white  overflow-y-auto transform transition-all duration-300 ease-in-out z-20",
          {
      ...
      >
        ...
        <ul className="mt-6 space-y-4">
          <li className="flex items-center">
            <Link
              href="/profile"
              className="w-full text-gray-700 hover:text-green-500 hover:bg-green-50 flex items-center rounded-md px-2 py-2 transition-colors"
            >
